...
def parse_args():
    ...
    parser.add_argument('-w',
                        dest="wallet_address",
                        help="get balances for a given wallet address")
    args = parser.parse_args()
    return args


def print_umbria_apy(umbria, coingecko, network):
    umbr_apy = umbria.get_umbria_apy(network)
    print("\n-------------------------------------------")
    print(f"Umbr APY on {network} network: {umbr_apy*100:.2f}%\n")
...
def print_balances_for_wallet(wallet_address, umbria, coingecko, network):
    ...
    token_prices = {
        token: coingecko.get_token_price_by_symbol(token)
        for token in tokens
    }
    staked_bal = umbria.get_all_staked(wallet_address)
    staked_bal_usd = {
        token: bal * token_prices[token]
        for token, bal in staked_bal.items()
    }
...
def main(wallet_address: str = None):
    ...
    print_umbria_apy(umbria, coingecko, network)
    if wallet_address:
